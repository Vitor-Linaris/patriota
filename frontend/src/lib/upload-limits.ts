 ...
 *   1. Backend: MEDIA_MAX_UPLOAD_BYTES in backend/.env (currently 10 MB).
 *      Multer rejects with 413 LIMIT_FILE_SIZE.
...
export function validateImageUpload(file: File): string | null {
  ...
  if (file.size > MAX_UPLOAD_BYTES) {
    return `Imagem demasiado grande: ${formatBytes(file.size)}. O limite é ${formatBytes(MAX_UPLOAD_BYTES)}. Reduza a imagem (por exemplo em tinypng.com) e tente novamente.`;
  }
  return null;
}
